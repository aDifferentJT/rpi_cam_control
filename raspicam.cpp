...
RaspiCam::RaspiCam(std::function<void(uint8_t const*, uint32_t)> callback) : callback(callback) {
  bcm_host_init();

  camera_component = create_camera_component();
  if (!camera_component) { g_warning("no camera component"); }

  {
    auto [component, pool] = create_encoder_component();
    encoder_component = component;
    encoder_pool = pool;

    if (!encoder_component) { g_warning("no encoder component"); }
    if (!encoder_pool) { g_warning("no encoder pool"); }
  }
  ...
  if (!camera_video_port) { g_warning("no camera video port"); }
  if (!encoder_input_port) { g_warning("no encoder input port"); }
  if (!encoder_output_port) { g_warning("no encoder output port"); }

  encoder_connection = RaspiCam::connect_ports(camera_video_port, encoder_input_port);
  if (!encoder_connection) { g_warning("no encoder connection"); }

  encoder_output_port->userdata = reinterpret_cast<MMAL_PORT_USERDATA_T*>(this);

  mmal_port_enable(encoder_output_port, mmal_encoder_buffer_callback);

  for (int q = 0; q < mmal_queue_length(encoder_pool->queue); q++) {
    auto buffer = mmal_queue_get(encoder_pool->queue);
...
MMAL_COMPONENT_T *RaspiCam::create_camera_component() {
  ...
  if ( auto status = raspicamcontrol_set_stereo_mode(camera->output[MMAL_CAMERA_VIDEO_PORT], &camera_parameters.stereo_mode)
     ; status != MMAL_SUCCESS
     ) {
  ...
  if ( auto status = mmal_port_parameter_set_uint32(camera->control, MMAL_PARAMETER_CAMERA_CUSTOM_SENSOR_CONFIG, sensor_mode)
     ; status != MMAL_SUCCESS
     ) {
  ...
  //  set up the camera configuration
  {
    MMAL_PARAMETER_CAMERA_CONFIG_T cam_config =
      ...
      , .num_preview_video_frames = (uint32_t)(3 + vcos_max(0, (framerate-30)/10))
  ...
  raspicamcontrol_set_all_parameters(camera.get(), &camera_parameters);
...
std::pair<MMAL_COMPONENT_T*, MMAL_POOL_T*> RaspiCam::create_encoder_component() {
  ...
  if (level == MMAL_VIDEO_LEVEL_H264_4) {
  ...
  if (intraperiod) {
    MMAL_PARAMETER_UINT32_T param = {{ MMAL_PARAMETER_INTRAPERIOD, sizeof(param)}, (uint32_t)*intraperiod};
  ...
  if (quantisationParameter) {
    ...
    {
      MMAL_PARAMETER_UINT32_T param = {{ MMAL_PARAMETER_VIDEO_ENCODE_MIN_QUANT, sizeof(param)}, quantisationParameter};
      if ( auto status = mmal_port_parameter_set(encoder_output, &param.hdr)
         ; status != MMAL_SUCCESS
         ) {
    ...
    {
      MMAL_PARAMETER_UINT32_T param = {{ MMAL_PARAMETER_VIDEO_ENCODE_MAX_QUANT, sizeof(param)}, quantisationParameter};
      if ( auto status = mmal_port_parameter_set(encoder_output, &param.hdr)
         ; status != MMAL_SUCCESS
         ) {
  ...
  if ( auto status = mmal_port_parameter_set_boolean(encoder_output, MMAL_PARAMETER_VIDEO_ENCODE_INLINE_HEADER, bInlineHeaders)
     ; status != MMAL_SUCCESS
     ) {
  ...
  if ( auto status = mmal_port_parameter_set_boolean(encoder_output, MMAL_PARAMETER_VIDEO_ENCODE_SPS_TIMING, addSPSTiming)
     ; status != MMAL_SUCCESS
     ) {
...
void RaspiCam::mmal_encoder_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *mmal_buffer) {
  RaspiCam *raspicam = reinterpret_cast<RaspiCam*>(port->userdata);

  auto raiiBufferRelease = gsl::finally([&] () {
    mmal_buffer_header_release(mmal_buffer);
    mmal_return_buffer_to_port(port, raspicam->encoder_pool->queue);
  });
  ...
  raspicam->callback(mmal_buffer->data, mmal_buffer->length);
}
